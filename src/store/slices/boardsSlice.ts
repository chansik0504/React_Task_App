...
const boardSlice = createSlice({
    ...
    reducers: {
        ...
        addTask: (state, {payload}: PayloadAction<TAddTaskAction>) => {
            state.boardArray.map(board => board.boardId === payload.boardId ?
                {...board, lists: board.lists.map(list => list.listId === payload.listId ?
                    {...list, task: list.tasks.push(payload.task)} : list)}
                    : board)
        },
...
export const {sort, addBoard, deleteList, deleteTask, updateTask, setModalActive, addTask, addList, deleteBoard} = boardSlice.actions;
